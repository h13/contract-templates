...
- [x] **Word → Markdown 変換**
  - Pandoc を使った変換の実装
  - 手動スクリプト（scripts/import-word.sh）
  - GitHub Actions での自動化

- [x] **PDF → Markdown 変換**
  - pdftotext による変換
  - 手動スクリプト（scripts/import-pdf.sh）
  - GitHub Actions での自動化

- [x] **双方向ワークフローの確立**
  ```
  既存文書 → Markdown → 編集 → Word/PDF/HTML
         ↑                      ↓
         └───────── フィードバックループ ─────┘
  ```
...
- [x] **GitHub Actions での自動化**
  - imports/ ディレクトリへのファイル追加で自動変換
  - PR自動作成
  - textlint自動チェック
...
- 2026-02-26: **Phase 1完了** - CI/CD完全動作、テンプレート8種類追加
- 2026-02-26: **Phase 2完了** - 双方向変換機能実装、GitHub Actions自動化
- 2026-02-26: **Phase 4部分完了** - Word/PDF/HTML出力対応
- ロードマップ策定完了
